...
- convolutional neural network 卷积神经网络 --> 擅于图像识别
- Long short-term memory network 长短期记忆网络 --> 擅于语音识别
- 多层感知器 MLP
- system prompt
- 第一段对话 : 用户的出示问题/提示词
...
- cost function 代价函数(告诉电脑预测结果的好坏及严重程度) :
  - 单个样本上上代价也叫做 `Loss / 损失 / 误差`, By 吴恩达.
    - 不过现在也有很多人混着用, 即 `loss function == const function`
  - 典型算法 :
...
- 每个词都对应一个 vec, 决定第一步中每个单词对应的向量
  ...
  - 即 : $W_E = \left\{ \mathbf{v}_i \right\}_{i=1}^{50257}, \quad \mathbf{v}_i \in \mathbb{R}^{12288}$
...
- 每次模型输出的矩阵中, 利用最后一个列向量和一个 unembedding-matrix(解嵌入矩阵) $W_U$ 映射到 50k 的 token 列表中
  ...
  - ${\color{red}{W_U}} = \left\{ \mathbf{v}_i \right\}_{i=1}^{12288}, \quad \mathbf{v}_i \in \mathbb{R}^{50257}$
